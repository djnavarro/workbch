...
Jobs can consist of multiple people on a *team* but the job must have a single *owner*, a named team member who is responsible for the project. When used in conjunction with nicknames, the `set_job()` function allows you to specify the team efficiently:
...
set_job("hitmebaby", "Signal detection modelling", "britney", priority = 2)
set_job("spinspinsugar", "Check for periodicities", "sneaker", priority = 1)
set_job("boys", "Distributional assumptions", "lizzo", priority = 2)
```
```{r busystate}
view_jobs()
```
...
More generally, `view_jobs()` and `view_priorities()` both allow you to pass filtering expressions to `dplyr::filter()` to extract the subset you're interested in. Suppose I only want to see the high priority active projects:
...
Often it is handy to add small annotations to a job. The intent here is not to use this as a substitute for proper documentation (that should happen within the project itself) but as a quick and dirty "notes to self" tool. You can add jobs using `set_note()`:
